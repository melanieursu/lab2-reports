...
public class MainWindow {
    ...
    private Button btnAddLine, btnEditLine, btnDelLine, btnHistory;
    ...
    private VBox buildDetailPanel() {
        ...
        btnHistory  = new Button("📈 История изменений");
        btnFinalize = new Button("📋 FINAL");
        btnSign     = new Button("✅ Подписать");

        HBox detailBtns = new HBox(6,
                btnAddLine, btnEditLine, btnDelLine, btnHistory,
        ...
        btnHistory.setOnAction(e -> {
            ReportLine selLine = lineTable.getSelectionModel().getSelectedItem();
            if (selLine == null) { showError("Выберите строку, чтобы посмотреть историю изменений"); return; }
            try {
                List<ru.itmo.TolstovaUrsu.domain.ReportLineHistory> history =
                        service.getLineHistory(selLine.getId());
                new LineHistoryDialog(selLine.getId(), history).showAndWait();
            } catch (ValidationException ex) { showError(ex.getMessage()); }
        });
